...
Carga eficiente de grandes volúmenes de datos mediante formato Parquet.
...
Pandas & PyArrow: (Manipulación de datos y manejo de Parquet)

DeepFace & TensorFlow: (Inteligencia Artificial para Visión Artificial)
...
Detector_de_Emociones.ipynb: Notebook principal que contiene todo el flujo: carga de Parquet, limpieza de texto, entrenamiento y el módulo de visión artificial.

Data/:

train.parquet & validation.parquet: Datasets procesados para el entrenamiento del modelo NLP.

Imagenes/: Carpeta local para alojar las muestras (.jpg, .png) utilizadas en las pruebas de reconocimiento facial.
...
Nota: Al ejecutar por primera vez, el módulo de visión descargará automáticamente los pesos de los modelos pre-entrenados (~500MB).